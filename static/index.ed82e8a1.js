...
})({"bwJdZ":[function(require,module,exports) {
...
function submitData() {
    const total_messages1 = chats.querySelectorAll(".message").length;
    ...
    if (userMessage.trim() !== "") {
        ...
        if (chats.innerHTML.trim() === "") botResponse.innerHTML = "<img style=\"opacity:0\" src='/r_logo.e040c57e.png'/>\n<p>Loading ... </p>";
        else botResponse.innerHTML = "<img src='/r_logo.e040c57e.png'/>\n<p>Loading ....  </p>";
        ...
        const firstChat = chats.querySelector(".message.bot");
        if (firstChat && total_messages1 < 2) {
            const firstChatLogo = firstChat.querySelector("img");
            const rect = firstChatLogo.getBoundingClientRect();
            const translateX = rect.left - bg_logo.offsetLeft;
            const translateY = rect.top - bg_logo.offsetTop;
...
input_box.addEventListener("keydown", function(event) {
    ...
    if (event.shiftKey && event.key === "Enter") {
        ...
        if (currentRows < maxRows) input_box.rows = Math.min(currentRows + 1, maxRows);
...
},{}]},["bwJdZ","cyUIL"], "cyUIL", "parcelRequire94c2")
